    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 6) {
                    AccentFillButton(title: "Scan network now") { model.beginScan() }
                    Button("Add by IP address…") {}.buttonStyle(.plain)
...
struct JustPairedView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            BrandHeader(title: "Spoolr Connect", subtitle: "\(model.workspace).spoolr.io") {
                StatusBadge(text: "1 LINKED")
            }

            VStack(spacing: 0) {
                SuccessRing().padding(.bottom, 10)
                Text("Connected.").font(Theme.sans(15, weight: .semibold)).foregroundColor(Theme.text1)
                Text("\(model.justPairedPrinter?.name ?? "Printer") is online and pushing telemetry to the dashboard.")
            ...
            if let p = model.justPairedPrinter {
            ...
            HairlineDivider()
            VStack(spacing: 0) {
                ActionRow(symbol: "plus", title: "Add another printer", shortcut: "⌘N", primary: true) { model.beginScan() }
