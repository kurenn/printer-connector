...
- **`internal/driver`** — the **protocol seam**. `Driver` is the only way the agent talks to a printer (Telemetry, QueryObjects, Pause/Resume/Cancel/Home/StartPrint, file ops, history, webcam, `Capabilities()`). `Telemetry` is the canonical, protocol-agnostic shape (`SchemaVersion = 1`, mirrors `spoolr-contracts/schemas/printer_telemetry.json`). **Adding a protocol means adding a `Driver` implementation — never branching on printer type across the agent or cloud.** `Capabilities()` gates which remote commands a printer accepts.
